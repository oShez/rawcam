...
host CMake test target, new `third_party/sse2neon/sse2neon.h`.
...
Every op above has an exact SSE4.1 equivalent under `sse2neon`
...
On the host this suite runs the vector path **via `sse2neon` (SSE)** and is the
...
- Host bit-exact across the full §6 matrix (vector path via `sse2neon`).
...
1. **`sse2neon` go/no-go spike:** vendor `third_party/sse2neon/sse2neon.h`; wire
   host CMake (`RAWV_USE_SSE2NEON`, `-msse4.1` or toolchain equivalent); prove
...
- **SIMD style: portable `std::experimental::simd` / `vector_size` (Option C)** —
  ...
  Chosen against in favor of Option B (`arm_neon.h` intrinsics + `sse2neon` host
