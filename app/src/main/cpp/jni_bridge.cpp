...
Java_com_shez_rawcam_NativeBridge_nativeStartRecording(
    ...
    jint requestedBitDepth, jlong startEpochNs, jint tzOffsetSec) {
  ...
  float colorMatrix2[9] = {0};
  if (jcolorMatrix2 != nullptr && env->GetArrayLength(jcolorMatrix2) >= 9) {
    jfloat tmp[9];
    env->GetFloatArrayRegion(jcolorMatrix2, 0, 9, tmp);
